...
if ($_POST["createOrder"] != NULL) {
	...
	if ($success) {
		...
		echo "<script language=\"javascript\" >alert(\"下单成功\"); history.go(-2); opener.location.reload();</script>";
	} else {
		echo "<script language=\"javascript\" >alert(\"下单失败，请联系店主手动下单\"); history.go(-2);</script>";
	}
}
